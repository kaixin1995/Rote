...
- **POSTGRES_IMAGE**
  データベースサービスで使用する PostgreSQL イメージです。既定値は `pgvector/pgvector:pg17-trixie` で、
  PostgreSQL 17 と同じように使え、AI セマンティック検索用の pgvector 拡張を追加します。
  通常の PostgreSQL を使い続けたい場合は次を設定してください：
